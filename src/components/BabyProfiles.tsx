...
import { Plus, Baby, X, Check } from 'lucide-react';
...
export default function BabyProfiles({ babies, selectedBabyId, onSelectBaby, onAddBaby }: BabyProfilesProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setBabyName(''); setBabyAge(3); setBabyGender('surprise'); setShowAddForm(false);
  };
          ...
          >
            <div className="p-4 bg-[#F7F5F2] rounded-xl border border-[#EAE6E0] space-y-3">
              ...
              <div className="flex gap-2 pt-1">
                <button id="baby-new-btn-cancel" type="button" onClick={() => setShowAddForm(false)} className="flex-1 py-2 text-xs font-medium text-[#7A716A] bg-white border border-[#E0DBD5] rounded-lg hover:bg-[#F5F2EE] cursor-pointer transition-all">Cancel</button>
        ...
        {babies.map((baby, idx) => {
          ...
          return (
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className={`relative p-3.5 rounded-xl border-2 transition-all cursor-pointer ${isSelected ? `${colors.activeBg} ${colors.activeBorder}` : `bg-white border-[#EAE6E0] hover:border-[#D5CFC8]`}`}
            >
              {isSelected && (
                <motion.div
